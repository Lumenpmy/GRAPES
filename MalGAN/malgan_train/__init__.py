...
class _DataGroup:  # pylint: disable=too-few-public-methods
    ...
    def build_loader(self, batch_size:0):
        r"""
...
class MalGAN(nn.Module):
    ...
    def __init__(self, mal_data: MalwareDataset, ben_data: MalwareDataset, Z: int,
        ...
        '''去除dataloader'''
        # print(MalGAN.MALWARE_BATCH_SIZE)
        ...
        #self._ben_data.build_loader(MalGAN.MALWARE_BATCH_SIZE)
    ...
    def fit(self, cyc_len: int, quiet_mode: bool = False) -> None:
        r"""
        Trains the model for the specified number of epochs. The model at the last epoch is used as the final model.
        ...
        for epoch_cnt in range(1, cyc_len + 1):
            ...
            # Save model if current validation loss is the best we've seen so far
            if flds[-1]:
                self._save(self._build_export_name(is_final=False))
                best_loss = valid_l_g

            if not quiet_mode:
                log.log(epoch_cnt, flds)

            # Save the final model after the last epoch
            if epoch_cnt == cyc_len:
                self._save(self._build_export_name(is_final=True))

        MalGAN.tensorboard.close()

        # Load the best model saved during training and save it as the final model
        self.load(self._build_export_name(is_final=False))
        self._save(self._build_export_name(is_final=True))
        self._delete_old_backup(is_final=False)

    def _build_export_name(self, is_final: bool = True) -> Path:
        r"""
    ...
    def measure_and_export_results(self) -> str:
        ...
        'batch-size=1'
        #y_mal_orig, m_prime_arr,y_hat_post,y_prob,bits_changed = [], [], [],[],[]
        # for m, _ in self._mal_data.test:
        #     y_mal_orig.append(self._bb._model.predict_features_proba(m.cpu()))
        #     if self._is_cuda:
        #         m = m.cuda()
        ...
        #
        #     m_diff = m_prime - m# 计算malAN添加的特征，也就是在原始的基础上加入的节、导入表等
        #     bits_changed.append(torch.sum(m_diff.cpu()))
        #
        #     # Sanity check no bits flipped 1 -> 0
        #     msg = "Malware signature changed to 0 which is not allowed"
        #     assert torch.sum(m_diff < -0.1) == 0, msg
        #     # assert torch.sum(m_diff < -0.1) == 0, msg 这个断言确保没有任何恶意签名的位从 1 变为 0
        #     # 因为这样的变化可能会削弱恶意软件的功能（malGAN的生成器中利用max函数来保证，生成的对抗性样本的功能在原本的恶意软件的样本上增加了一些无关的功能）
        #     # 通过这种方式，代码确保生成器不会产生任何对恶意软件功能有害的变化。
        #     # 这里使用 -0.1 作为阈值是为了处理浮点数计算中的小误差。
        # print(bits_changed)
        # bits_changed_tensors = [torch.tensor([x]) for x in bits_changed]  # 将每个标量转换为一维张量
        # print(bits_changed_tensors)
        # avg_changed_bits = torch.cat(bits_changed_tensors).mean()
        #
        # logging.debug("Avg. Malware Bits Changed Changed: %2f", avg_changed_bits)
        #
        # for m, _ in self._ben_data.test:
        #     y_mal_orig.append(self._bb._model.predict_features_proba(m.cpu()))
        # # BB prediction of the malware before the generator
        # y_mal_orig = [torch.tensor([x]) for x in y_mal_orig]
        ...
        # m_prime_tensor = torch.stack(m_prime_arr, dim=0)
        # ben_test_tensor = torch.stack(ben_test_arr, dim=0)
        # x = torch.cat([m_prime_tensor, ben_test_tensor], dim=0)# torch.Size([361, 128])
        #
        ...
        # for m in x:
        #     y_hat_post.append(self._bb.predict(m).item())
        #     y_prob.append(self._bb._model.predict_features_proba(m))  # pylint: disable=protected-access
        # y_prob = [torch.tensor([x]) for x in y_prob]
        # y_prob = torch.cat(y_prob)
        # y_hat_post = [torch.tensor([x]) for x in y_hat_post]
        # y_hat_post = torch.cat(y_hat_post)
        #
        # if self._is_cuda:
        #     y_mal_orig, y_hat_post, y_actual = y_mal_orig.cpu(), y_hat_post.cpu(), y_actual.cpu()
        # print(y_prob.shape,y_hat_post.shape,y_actual.shape,y_mal_orig.shape)
        # print(y_prob,y_hat_post,y_actual,y_mal_orig)
        # return _export_results(self, valid_loss, test_loss, avg_changed_bits, y_actual,
        #                        y_mal_orig, y_prob, y_hat_post)
        for m, _ in self._mal_data.test:
            y_mal=self._bb._model.predict_features_proba(m.cpu())
            y_mal_orig.append(y_mal)
            if self._is_cuda:
                m = m.cuda()
            num_mal_test += m.shape[0]
            #num_mal_test += 1
        ...
        logging.debug("Avg. Malware Bits Changed Changed: %2f", avg_changed_bits)
        y_mal_orig =  torch.cat(y_mal_orig)#torch.size([273])
        ben_test_arr = [x.cpu() if self._is_cuda else x for x, _ in self._ben_data.test]
        x = torch.cat(m_prime_arr + ben_test_arr)# torch.Size([361, 128])
        y_actual = torch.cat((torch.full((num_mal_test,), MalGAN.Label.Malware.value),
                             torch.full((len(x) - num_mal_test,), MalGAN.Label.Benign.value)))
        y_hat_post=self._bb.predict(x)
        y_prob=self._bb._model.predict_features_proba(x)  # pylint: disable=protected-access
