...
SECTION_INDEX = 6725# 这是特征向量中节开始的序列码？

# api = CuckooAPI.CuckooAPI("10.20.0.144", APIPY=True, port=8090)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("extract_features/feature_vector_directory/feature_vector_mapping.pk"),
    )
...
# 收集所有PE的对抗性样本中出现的新的导入表的名称，并除去不合格的
'''假设传入的内容为：
['CreateFile:kernel32', 'ReadFile:kernel32', 'MessageBox:user32', 'CreateWindow:user32']
输出：
字典（adversarial_imports_dict）：
{
    "kernel32": ["CreateFile", "ReadFile"],
    "user32": ["MessageBox", "CreateWindow"]
}'''
...
def binary_builder(
    malware_pe: str,# 原始的恶意软件样本
    adversarial_vector: str,# 对抗性样样本针对feature_type的特征向量
    feature_mapping: str,# 对应于feature_type的特征对应列表，每个特征代表的意义是什么
    output_path: str,
    feature_type: str,# 提取的原始恶意如那件的特征向量的类型，比方说节名
):
    ...
    number_of_mutated_files = 50# 允许接收的最大的变异数吗？
    ...
    if feature_type.lower() == "section":
        ...
        section_state = True
        # Mutated_Binaries/args.output_path/Sections
        output_path = os.path.join(output_path, "Sections")
        # Mutated_Binaries/args.output_dir/Sections
        if not os.path.exists(output_path):
        ...
        )# 得到对抗性样本的节名，并存入RL_features/adversarial_sections_set.pk
        ...
        # 创建一个为Sections的空的目录RL_features/Sections
        ...
        if not os.path.exists(RL_features_section):
            ...
            folder_contents = os.listdir(RL_features_section)# 得到RL_features_section路径下所有的文件
            folder_contents = [
        ...
        # 将对抗新样本中的前50个样本的节名纯如相应的RL_features/Sections/str(index)_adversarial_sections_set.txt路径下
        ...
        imports_state = True
        # Mutated_Binaries/args.output_dir/Imports
        ...
        RL_features_imports = os.path.join(RL_features, "imports")
        # RL_features/all_features/imports
        if not os.path.exists(RL_features_imports):
    ...
    try:
        ...
        for index in track(
            ...
            if imports_state:# 如果采用了导入表的特征
                ...
                ["section4", "section5"]]
                sections = ["section2", "section3", "section6"]
                feature_mapping=["section2", "section3", "section6","section8"]
                那么imports_to_be_added就是["section1"]'''
                debug(
                    f"\t\t[+] Number of imports to be added : {len(imports_to_be_added)}"
                )# 每个对抗性样本相对于所有的恶意如那件样本的导入表中需要增加的导入表

                if len(imports_to_be_added):# 如果对抗性样本增加的导入表里
                    ...
                    )# #Mutated_Binaries/args.output_dir/Imports/index mutated/
            ...
            if section_state:
                ...
                if len(sections_to_be_added):
                    ...
                    call_c_application_for_section(
                        ...
                        malware_pe,
                        str(filepath),
                        str(Path("extract_features/manipulation_content/section_content.txt")),
                        output_file,
                    )
...
def section_extractor(adversarial_vector: str, feature_mapping: str):
    ...
    debug(f"[+] adversarial_feature_vector length : {len(adversarial_feature_vector)}")
    # 因为adversarial_feature_vector是一个列表，每一个元素是每一个对抗性样本的关于节的特征向量
    ...
    for index in track(range(len(adversarial_feature_vector)), description="Mapping sections ...", transient=True):
        ...
        for i in track(
            ...
            sample = adversarial_feature_vector[index][i]#选取每一个对抗性样本的特征向量
            sample = sample.tolist()
            adversial_section = []
            ...
            adversarial_section_set.append(adversial_section)# 得到一个列表，每一个元素是每一一个PE的对抗性样本中的节名
            debug(
...
def main():
    ...
    if str(args.feature_vector).lower() == "section":
        ...
        feature_mapping = Path(
            "extract_features/feature_vector_directory/section_feature_vector_mapping.pk"
        )# 带有样本中所有PE文件的节名的特征向量图
    elif str(args.feature_vector).lower() == "imports":
        adversarial_vector = Path(
            "adversarial_feature_vector_directory/adversarial_imports_set.pk"
        )
        feature_mapping = Path(
            "extract_features/feature_vector_directory/import_feature_vector_mapping.pk"
        )
